...
众多周知，android开发中，每次编译APK的时间都是比较长的，尤其是在本地机器配置较差的情况下，编译一次的时间就更长了。通过`mainframer`，我们可以实现在本机开发的同时，使用一个远端机器(拥有高配置的机器)上编译代码，然后自动将编译结果同步到本地，再自动执行安装，调试等任务。
...
在编译过程中，会下载很多依赖包，由于众多周知的原因，我们需要使用各种源镜像才能流畅编译
